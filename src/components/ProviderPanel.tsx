    ...
    <div ref={ref} className="relative">
      ...
      >
        <span className="flex min-w-0 items-center gap-2.5">
          <span className="truncate text-sm text-zinc-100">{activePreset?.name ?? t('provider.select')}</span>
          {activePreset && (
            <span className={`mono-label shrink-0 rounded-sm border px-1.5 py-0.5 ${BADGE_STYLE[activePreset.badge]}`}>
              {t(BADGE_KEYS[activePreset.badge])}
            </span>
          )}
        </span>
        ...
        >
          <Command shouldFilter filter={providerFilter} loop>
            ...
            <CommandList className="scroll-thin max-h-64 overflow-y-auto p-1">
              ...
              <CommandGroup
                heading={t('provider.groupBuiltin')}
              ...
              {catalogStatus === 'ready' && catalog.length > 0 && (
                <CommandGroup
                  heading={t('provider.groupCatalog', { n: catalog.length })}
                  className="text-xs text-zinc-500 [&_[cmdk-group-heading]]:px-3 [&_[cmdk-group-heading]]:py-1.5 [&_[cmdk-group-heading]]:font-mono [&_[cmdk-group-heading]]:text-[10px] [&_[cmdk-group-heading]]:uppercase [&_[cmdk-group-heading]]:tracking-[0.18em] [&_[cmdk-group-heading]]:text-zinc-600"
                >
                  {catalog.map((p) => (
                    <ProviderItem key={p.id} preset={p} active={p.id === activeId} onSelect={(id) => { onSelect(id); setOpen(false) }} />
        ...
        >
          ...
          {listState.status === 'error' && (
            <div className="px-3 py-3 text-xs leading-relaxed text-red-400">
              ...
              {!canFetch && <span className="text-zinc-600">{t('provider.listNoFetch')}</span>}
            </div>
          )}
          ...
          {listState.status === 'ready' && filtered.length === 0 && (
            <div className="mono-label px-3 py-3.5 text-zinc-600">{t('provider.listNoMatch')}</div>
          )}
          {listState.status === 'idle' && !canFetch && (
            <div className="mono-label px-3 py-3.5 text-zinc-600">{t('provider.listUnsupported')}</div>
          )}
        </motion.div>
      )}
    </div>
            ...
            >
              <div className="mb-5 flex items-center justify-between">
                ...
                <Dialog.Close asChild>
                  <button type="button" className="btn-ghost rounded-sm p-1.5" aria-label={t('provider.close')}>
                    <X size={12} />
                  </button>
                </Dialog.Close>
              </div>
                ...
                <div className="col-span-2">
                  ...
                  {preset && (
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
                      ...
                      <span className="font-mono text-[11px] text-zinc-600">{preset.defaultBaseURL || t('provider.officialEndpoint')}</span>
                  ...
                  {testResult && (
                    ...
                    >
                      {testResult.ok
                        ...
                        : `✗ ${testResult.errorKey ? t(testResult.errorKey) : (testResult.error ?? t('provider.connectFailed'))}`}
                    </motion.p>
                  )}
                </div>
              </div>
