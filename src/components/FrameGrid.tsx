...
export function FrameGrid() {
  ...
  return (
    <div className="grid grid-cols-2 gap-2">
      {frames.map((f) => (
        <figure key={f.id} className="group overflow-hidden rounded-lg border border-zinc-800 bg-zinc-900/40">
          <img src={f.dataUrl} alt={`frame @ ${formatTime(f.timeSec)}`} className="w-full" loading="lazy" />
