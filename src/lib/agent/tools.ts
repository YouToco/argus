...
const timeRange = z.tuple([z.number(), z.number()])
...
export function buildToolRegistry(): ArgusTool[] {
  const tools: ArgusTool[] = [
    ...
    },
    {
      ...
      inputSchema: z.strictObject({
        start_seconds: z.number().describe('起始时间（秒）'),
        end_seconds: z.number().describe('结束时间（秒）'),
        interval_seconds: z.number().describe('抽帧间隔（秒）'),
        max_width: z.number().optional().describe('输出帧最大宽度像素，默认 640'),
        max_frames: z.number().optional().describe('最多抽取帧数，超限时均匀采样'),
        quality: z.number().optional().describe('JPEG 质量 0~1，默认 0.7'),
      }),
      execute: async (input: {
    ...
    },
    {
      ...
      inputSchema: z.strictObject({
        time_seconds: z.number().describe('时间点（秒）'),
        max_width: z.number().optional().describe('输出最大宽度像素，默认 1280'),
      }),
      execute: async (input: { time_seconds: number; max_width?: number }, ctx) => {
        if (!ctx.session) return { text: '尚未加载视频文件。' }
    ...
    },
    {
      ...
      inputSchema: z.strictObject({
        frame_id: z.string().describe('帧 id'),
        x: z.number().describe('区域左上角 x（0~1）'),
        y: z.number().describe('区域左上角 y（0~1）'),
        width: z.number().describe('区域宽度（0~1）'),
        height: z.number().describe('区域高度（0~1）'),
        scale: z.number().optional().describe('放大倍数，默认 2'),
      }),
      execute: async (input: {
        frame_id: string
        x: number
    ...
    },
    {
      ...
      inputSchema: z.strictObject({
        ...
        topic: z.string().describe('主题，如"人数"、"车辆"、"异常事件"'),
        notes: z.string().describe('该时间段观察到的结论'),
      }),
      execute: async (input: { start_seconds: number; end_seconds: number; topic: string; notes: string }, ctx) => {
    ...
    },
    {
      ...
      inputSchema: z.strictObject({
        ...
        time_range: timeRange.optional().describe('[起始秒, 结束秒]'),
        query: z.string().optional().describe('在记录内容里搜索的关键词'),
      }),
      execute: async (input: { topic?: string; time_range?: [number, number]; query?: string }, ctx) => {
    ...
    },
    {
      ...
      inputSchema: z.strictObject({
        time_range: timeRange.describe('[起始秒, 结束秒]'),
        goal: z.string().describe('该子代理要达成的分析目标'),
        max_frames: z.number().optional().describe('子代理最多抽帧数，默认 12'),
        max_steps: z.number().optional().describe('子代理最大步数，默认 8'),
      }),
      execute: async (input: { time_range: [number, number]; goal: string; max_frames?: number; max_steps?: number }, ctx) => {
